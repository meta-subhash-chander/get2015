...
public class GeneratePermutations {
	...
	public String permutestring(String beginningstring, String endingstring) {
		if (endingstring.length() <= 1) {
			...
			for (int count = 0; count < endingstring.length(); count++) {
				String newString = endingstring.substring(0, count)
						+ endingstring.substring(count + 1);
				outputpermutestring += permutestring(beginningstring
						+ endingstring.charAt(count), newString);
			}
	...
	/** Start  main method
	 * @param: String inputstring :Specify the input string that have topermuted
 ...
 */
	public static void main(String args[]) {
		...
		System.out.println("Enter a string that have char.\n");
		try {
